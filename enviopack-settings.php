...
{   
    ...
    echo '<input type="text" name="api_key" value="' . (esc_html($previous_config) ? esc_html($previous_config) : '') . '" />';
}

function print_api_secret()
{
    $previous_config = get_option('enviopack_api_secret');    
    echo '<input type="text" name="api_secret" value="' . (esc_html($previous_config) ? esc_html($previous_config) : '') . '" />';
}
...
function print_epack_api_link()
{
    ...
    <br><strong>' . esc_url(get_site_url(null, '/wc-api/ecom-enviopack')) . '</strong></p>';
}

function print_branch_office()
{
    $previous_config = get_option('enviopack_branch_office');    
    echo '<input type="text" name="branch_office" value="' . (esc_html($previous_config) ? esc_html($previous_config) : '') . '" />';
...
function print_packaging_mode()
{
    ...
    echo '<option value="sum-package" ' . (esc_html($previous_config) === 'sum-package' ? 'selected' : '') . '>Calcular sumando las dimensiones de los productos</option>';
    echo '<option value="max-package" ' . (esc_html($previous_config) === 'max-package' ? 'selected' : '') . '>Calcular tomando la dimension mas alta de cada producto</option>';
    echo '<option value="default-package" ' . (esc_html($previous_config) === 'default-package' ? 'selected' : '') . '>Paquete default</option>';
    echo '</select>';
...
function print_shipping_mode()
{
    if (get_option('enviopack_api_key') && get_option('enviopack_api_secret')) {
        ...
        echo '<option value="manual" ' . (!esc_html($previous_config) || esc_html($previous_config) === 'manual' ? 'selected' : '') . '>Enviar manualmente</option>';
        foreach ($couriers as $courier) {
            echo '<option value="' . esc_html($courier['id']) . '" ' . (esc_html($previous_config) === esc_html($courier['id']) ? 'selected' : '') . '>Enviar automaticamente - ' . esc_html($courier['name']) . '</option>';
        }
        echo '</select>';
...
function print_default_shipping_status()
{
    ...
    foreach ($statuses as $status_key => $status_name) {
        if ($previous_config) {
            echo '<option value="' . esc_html($status_key) . '" ' . (esc_html($previous_config) === esc_html($status_key) ? 'selected' : '') . '>' . esc_html($status_name) . '</option>';
        } else {
            echo '<option value="' . esc_html($status_key) . '" ' . (esc_html($status_key) === 'wc-completed' ? 'selected' : '') . '>' . esc_html($status_name) . '</option>';
        }
    }
    echo '</select>';
        ...
        foreach ($addresses as $address) {
            if ($previous_config) {
                if ($previous_config === $address['id']) {
                    echo '<option value="' . esc_html($address['id']) . ' selected">' . esc_html($address['address']) . '</option>';
                } else {
                    echo '<option value="' . esc_html($address['id']) . '">' . esc_html($address['address']) . '</option>';
                }
            } else {
                if ($address['default']) {
                    echo '<option value="' . esc_html($address['id']) . ' selected">' . esc_html($address['address']) . '</option>';
...
function print_google()
{
    ...
    echo '<input type="text" name="gmap_key" value="' . (esc_html($previous_config) ? esc_html($previous_config) : '') . '" />';
...
function print_order_status_on_processed()
{
    ...
    foreach ($statuses as $status_key => $status_name) {
        if ($previous_config) {
            echo '<option value="' . esc_html($status_key) . '" ' . (esc_html($previous_config) === $status_key ? 'selected' : '') . '>' . esc_html($status_name) . '</option>';
        } else {
            echo '<option value="' . esc_html($status_key) . '" ' . (esc_html($status_key) === 'wc-completed' ? 'selected' : '') . '>' . esc_html($status_name) . '</option>';
        }
    }
...
function settings_page_content()
{
    ...
    if (isset($_POST['api_key'])) {
        update_option('enviopack_api_key', sanitize_key($_POST['api_key']));
    }

	// Save api_secret
    if (isset($_POST['api_secret'])) {
        update_option('enviopack_api_secret', sanitize_key($_POST['api_secret']));
    }
    
	// Save address id
    if (isset($_POST['address'])) {
        update_option('enviopack_address_id', sanitize_key($_POST['address']));
    }
    
    // Save branch_office
    if (isset($_POST['branch_office'])) {
        update_option('enviopack_branch_office', sanitize_text_field($_POST['branch_office']));
    }

	// Save google maps api key
    if (isset($_POST['gmap_key'])) {
        update_option('enviopack_gmap_key', sanitize_text_field($_POST['gmap_key']));
    }

	// Save shipping mode
    if (isset($_POST['shipping_mode'])) {
        update_option('enviopack_shipping_mode', sanitize_key($_POST['shipping_mode']));
    }

	// Save packaging mode
    if (isset($_POST['packaging_mode'])) {
        update_option('enviopack_packaging_mode', sanitize_key($_POST['packaging_mode']));
    }

	// Save shipping status
    if (isset($_POST['shipping_status'])) {
        update_option('enviopack_shipping_status', sanitize_key($_POST['shipping_status']));
    }

	// Save debug
    if (isset($_POST['debug'])) {
        update_option('enviopack_debug', sanitize_text_field($_POST['debug']));
    }

    // Save order status
    if (isset($_POST['status_on_processed'])) {
        update_option('enviopack_status_on_processed', sanitize_key($_POST['status_on_processed']));
    }
